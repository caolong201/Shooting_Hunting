...
namespace IE.RSB
{
    ...
    public class BulletTimeCameraDefault : MonoBehaviour
    {
        ...
        private void BulletTimeStarted(Transform bullet, Transform hitTarget, ref List<BulletPoint> bulletPath, float totalDistance)
        {
            ...
            Debug.Log("BulletTimeStarted: " + m_cameraEffects[m_selectedEffect].m_name);
        ...
        private IEnumerator StartStage()
        {
            ...
            if (stage.m_interpolatePosition)
            {
                ...
                // Fix: giữ khoảng cách khi kết thúc position transition
                if (stage.m_positionType == CameraEffectStage.PositionType.OnBullet)
                {
                    if (targetEnd.z > -minFollowDistance)
                    {
                        targetEnd.z = -minFollowDistance;
                    }
